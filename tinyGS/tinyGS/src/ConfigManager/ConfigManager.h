...
typedef struct
{
  ...
  // {"adcVoltIntPIN": 34, "volCoefInt": 0.0029626418, "adcVoltExtPIN": 35 , "volCoefExt": 0.0075517320}

  int adcVoltIntPIN = -1;
  int adcVoltExtPIN = -1;

  float volCoefInt = 0; // example : 0.0029626418;
  float volCoefExt = 0; // example : 0.0075517320;
...
public:
  ...
  bool getBoardConfig(board_t &board)
  {
    ...
    {
      currentBoard = boards[getBoard()];
    }
    else
    {
      ret = parseBoardTemplate(currentBoard);
    }
