...
BatteryMonitor *onboardBattMon;
BatteryMonitor *externalBattMon;

// static const float bm_coeff = 3.28 / 1107.12;
// static const float bmext_coeff = 12.93 / 1712.19;
...
void ConfigManager::handleDashboard()
{
  ...
  svg += "<circle id="
         ...
         " cx="
         "" +
         String(status.satPos[0] * 2 + 3) + ""
                                            " cy="
                                            "" +
         String(status.satPos[1] * 2 + 3) + ""
                                            " stroke="
                                            "red"
                                            " fill="
                                            "none"
  ...
  s += F("</table></div><div class=\"card\"><h3>Groundstation Status</h3><table id="
         "gsstatus"
         ">");
  ...
  if (onboardBattMon->isActive() and externalBattMon->isActive())
  {
    s += "<tr><td>Power supply </td><td>" +
         String(onboardBattMon->getVoltage()) + " / " +
         String(externalBattMon->getVoltage()) + " V</td></tr>";
  }
  else if (onboardBattMon->isActive())
  {
    s += "<tr><td>Power supply </td><td>" +
         String(onboardBattMon->getVoltage()) + " V (on board)</td></tr>";
  }
  else if (externalBattMon->isActive())
  {
    s += "<tr><td>Power supply </td><td>" +
         String(externalBattMon->getVoltage()) + " V (external)</td></tr>";
  }

  s += F("</table></div>");
  s += F("<div class=\"card\"><h3>Modem Configuration</h3><table id="
         "modemconfig"
         ">");
  ...
  s += F("</table></div><div class=\"card\"><h3>Last Packet Received</h3><table id="
         "lastpacket"
         ">");
...
boolean ConfigManager::init()
{
  ...
  onboardBattMon = new BatteryMonitor(advancedConf.adcVoltIntPIN, advancedConf.volCoefInt);
  externalBattMon = new BatteryMonitor(advancedConf.adcVoltExtPIN, advancedConf.volCoefExt);

  if (onboardBattMon->isActive()) {
   Log::console(PSTR("Internal power supply monitor - (PIN:%u/Coeff:%f)"),advancedConf.adcVoltIntPIN, advancedConf.volCoefInt );
  }
    if (externalBattMon->isActive()) {
   Log::console(PSTR("External power supply monitor - (PIN:%u/Coeff:%f)"),advancedConf.adcVoltExtPIN, advancedConf.volCoefExt);
  }
...
void ConfigManager::parseAdvancedConf()
{
  ...
  if (doc.containsKey(F("volCoefInt")))
  {
    advancedConf.volCoefInt = doc["volCoefInt"];
  }
  if (doc.containsKey(F("volCoefExt")))
  {
    advancedConf.volCoefExt = doc["volCoefExt"];
  }

  if (doc.containsKey(F("adcVoltIntPIN")))
  {
    advancedConf.adcVoltIntPIN = doc["adcVoltIntPIN"];
  }
  if (doc.containsKey(F("adcVoltExtPIN")))
  {
    advancedConf.adcVoltExtPIN = doc["adcVoltExtPIN"];
  }
}
